...
export const getCart = async (req: Request, res: Response) => {
  try {
    ...
    const sessionId = (req as any).sessionID; // Type assertion for session

    const whereCondition = userId
      ? { userId, status: 'cart' as const }
      : { guestSessionId: sessionId, status: 'cart' as const };
    ...
    const cartItems = cartOrder ? cartOrder.items.map((item: any) => ({
