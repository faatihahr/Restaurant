...
export const register = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // If no profile image uploaded, generate a random avatar
    if (!profileImageUrl) {
      try {
        // Generate random seed based on user email for consistent avatar
        const seed = email.split('@')[0] + Date.now().toString();
        ...
        // Create a file-like object for the avatar
        const avatarBuffer = Buffer.from(response.data);
        const avatarFileName = `avatar-${Date.now()}.svg`;
        ...
        profileImageUrl = `profiles/${avatarFileName}`;
        console.log('Generated random avatar:', profileImageUrl);
      } catch (error) {
